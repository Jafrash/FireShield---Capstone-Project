  ...
  template: `
    <div class="page-container">
      <div class="page-header">
        ...
        <p class="text-sm text-gray-600 mt-1">View and manage all property inspections</p>
      </div>
      ...
      @if (loading()) {
        ...
        <div class="content-card">
          <div class="text-center py-12">
            ...
            <h3 class="mt-4 text-lg font-medium text-gray-900">No Property Inspections</h3>
            <p class="mt-2 text-sm text-gray-500">Property inspections will appear here when surveyors are assigned.</p>
          </div>
        </div>
      } @else {
        <div class="content-card">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                @for (inspection of inspections(); track inspection.inspectionId) {
                  <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">#{{inspection.propertyId}}</td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      @if (inspection.assessedRiskScore !== null && inspection.assessedRiskScore !== undefined) {
                        <span [class]="getRiskScoreClass(inspection.assessedRiskScore)">
                          {{inspection.assessedRiskScore}}/100
                        </span>
                      } @else {
        ...
        <div class="modal-overlay" (click)="closeDetails()">
          <div class="modal-content" (click)="$event.stopPropagation()">
            <div class="modal-header">
              <h2 class="text-xl font-bold text-gray-900">Property Inspection Details</h2>
            ...
            <div class="modal-body">
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-500">Property ID</label>
                  <p class="mt-1 text-base text-gray-900">#{{selectedInspection()!.propertyId}}</p>
                </div>
                <div>
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-500">Risk Score</label>
                  <p class="mt-1">
                    @if (selectedInspection()!.assessedRiskScore !== null && selectedInspection()!.assessedRiskScore !== undefined) {
                      <span [class]="getRiskScoreClass(selectedInspection()!.assessedRiskScore!)">
                        {{selectedInspection()!.assessedRiskScore}}/100
                      </span>
                    } @else {
                ...
                @if (selectedInspection()!.remarks) {
                  <div class="col-span-2">
                    ...
                    <p class="mt-1 text-base text-gray-700 whitespace-pre-wrap">{{selectedInspection()!.remarks}}</p>
                  </div>
                }
              </div>
            </div>
...
export class PropertyInspectionsComponent implements OnInit {
  ...
  inspections = signal<InspectionDetail[]>([]);
  loading = signal(false);
  error = signal<string | null>(null);
  selectedInspection = signal<InspectionDetail | null>(null);
  ...
  loadInspections() {
    ...
    this.adminService.getAllInspections().subscribe({
      ...
      error: (err) => {
        ...
        this.error.set('Failed to load inspections. Please try again.');
        this.loading.set(false);
      }
    });
  }

  viewDetails(inspection: InspectionDetail) {
  ...
  getRiskScoreClass(score: number): string {
    ...
    if (score >= 80) return base + 'text-red-600';
    if (score >= 60) return base + 'text-orange-600';
    if (score >= 40) return base + 'text-yellow-600';
